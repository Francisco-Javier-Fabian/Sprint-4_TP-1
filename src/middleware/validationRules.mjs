...
export const validationDataSuperHeros = () => [
    ...
    .isLength({ min: 3, max: 60 }).withMessage('El nombre superheoristico debe tener entre 3 y 60 caracteres per favore').trim(),
    body('nombreReal').notEmpty().withMessage('Nombre Real es requerido')
    .isLength({ min: 3, max: 60 }).withMessage('El nombre real debe tener entre 3 y 60 caracteres per favore').trim(),
    body('edad').notEmpty().withMessage('La edad es requerido')
    .isInt({ min: 0 }).withMessage('Edad es incorrecta').trim(),
    ...
    .isLength({ min: 3, max: 60 }).withMessage('Cada poder debe tener entre 3 y 60 caracteres per favore').trim()
]